...
#  - VALGRIND_CHECKTARGET: Name of makefile target to execute under valgrind.
#    (Default: check-TESTS)
...
VALGRIND_CHECKTARGET ?= check-TESTS
...
valgrind_non_dflt_target = $(if $(filter-out check-TESTS,$(VALGRIND_CHECKTARGET)),1)
...
ifneq ($$(TESTS)$$(valgrind_non_dflt_target),)
	$$(valgrind_v_use)$$(MAKE) $(VALGRIND_CHECKTARGET) \
